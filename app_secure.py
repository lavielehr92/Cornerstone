...
if not sites_df.empty:
    ...
    if hide_exact:
        ...
        def to_zip_centroid(addr, lat, lon):
            pc = nomi.query_postal_code(geolocator.reverse((lat, lon), exactly_one=True, language="en").raw.get("address", {}).get("postcode") if (lat and lon) else None)
        ...
        def jitter_point(la, lo, meters=150):
            if pd.isna(la) or pd.isna(lo):
                return la, lo
            # Rough conversion: 1 deg lat ~ 111_111 m, lon scaled by cos(lat)
...
# Add ZCTA circles
radius_mm = minmax(work["desert_score"])
